...
const start = async () => {
    puppeteer.default
        ...
        const selectExists = !!(await page2.$("#listing-page-cart .wt-select select"));
        // NOTE Get ID selector and option field value
        if (selectExists) {
            const options = await page2.$$eval("#listing-page-cart .wt-select select", (rows) => {
                return rows.map((row) => {
                    return {
                        selector: `#${row.getAttribute("id")}`,
                        value: [...row.querySelectorAll("option")][1].getAttribute("value"),
                    };
                });
            });
        ...
        await page2.waitForSelector(".new-buyer-shipping-form #shipping-form-email-input");
        await page2.waitForSelector(".new-buyer-shipping-form #shipping-form-email-confirmation");
        await page2.waitForSelector(".new-buyer-shipping-form input[data-field='name']");
        await page2.waitForSelector(".new-buyer-shipping-form input[name='first_line']");
        await page2.waitForSelector(".new-buyer-shipping-form input[data-field='city']");
        await page2.waitForSelector(".new-buyer-shipping-form input[data-field='zip']");
        await page2.waitForSelector("button[data-selector-save-btn]");
        // NOTE Fill input fields !!!
        const email = await page2.$(".new-buyer-shipping-form #shipping-form-email-input");
        await email.type("someuser@gmail.com");
        const confirmEmail = await page2.$(".new-buyer-shipping-form #shipping-form-email-confirmation");
        await confirmEmail.type("someuser@gmail.com");
        const name = await page2.$(".new-buyer-shipping-form input[data-field='name']");
        await name.type("John Doe");
        const address = await page2.waitForSelector(".new-buyer-shipping-form input[data-field='first_line']");
        await address.type("Some Random Street");
        const city = await page2.waitForSelector(".new-buyer-shipping-form input[data-field='city']");
        await city.type("Miami");
        const zip = await page2.waitForSelector(".new-buyer-shipping-form input[data-field='zip']");
